...
//		$this->db->where ( 'T.STATUS_AKTIF <>', 3 );
	...
	public function getIdReimpay(){
		$sql= "select id_reimpay from master_reimpay";
		$query = $this->db->query($sql);
		$jml = $query->num_rows();
		if($jml == 0){
			$id_reimpay = "000001";
			...
			$sql= "select max(right(id_reimpay,6)) as id_reimpay from master_reimpay";
			$query = $this->db->query($sql);
			$hasil = $query->result();
			$id_reimpay =  $hasil[0]->id_reimpay;
			$id_reimpay = sprintf('%06u',$id_reimpay+1);
			return $id_reimpay;
		}
	}
